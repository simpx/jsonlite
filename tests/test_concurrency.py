...
def test_concurrent_writes(temp_db):
    ...
    p1 = multiprocessing.Process(target=multi_insert, args=(filename, records1))
    p2 = multiprocessing.Process(target=multi_insert, args=(filename, records2))
...
def test_concurrent_reads(temp_db):
    ...
    db.insert_many(records)
    processes = [multiprocessing.Process(target=multi_read, args=(filename,)) for _ in range(10)]
...
def test_read_while_writing(temp_db):
    ...
    p_write = multiprocessing.Process(target=multi_insert, args=(filename, records_to_insert))
    ...
    result = db.find({})
    assert len(result) in [2, 4]  # Depending on timing, there might be a temporary state
...
def test_write_while_reading(temp_db):
    ...
    p_write = multiprocessing.Process(target=multi_insert, args=(filename, records_to_insert))
    p_read = multiprocessing.Process(target=multi_read_with_sleep, args=(filename,))
    p_read.start()
    ...
    result = db.find({})
    assert len(result) == 4  # Reading should be eventually consistent with all writes
...
def start_and_kill_processes(filename, process_count, record_count):
    ...
    for _ in range(process_count):
        p_insert = multiprocessing.Process(target=insert_records, args=(filename, record_count))
        p_insert.start()
